...
<html>
	...
	<body>
                        ...
                        if (c.assignee == undefined || c.assignee == "") {
                        	...
                        	if (c.formKey != undefined && c.formKey != null && c.formKey != "") {
                        		...
                        		for (var i = 0; i < infos.length; i++) {
                        			...
                        			} else if (infos[i] == "newUser") {
                        				func.push({"fn": "viewUserInfoById('" + c.newCustomerId + "','" + c.newUserId + "','" + c.newBusinessKey + "')", "color": "red", "icon": "user-md", "tooltip": "查看新用户信息"});
                        			} else if (infos[i] == "newRoute") {
                        				func.push({"fn": "goToPortRoute('" + c.newUserId + "')", "color": "purple", "icon": "road", "tooltip": "重新配线"});
                        			}
                        		}
                        	}
		...
		// 查看用户信息
		function viewUserInfoById(customerId, userId, userKey) {
			loadCustomerInfo(customerId);
			loadAccountInfo(customerId);
			loadUserInfo(userId);
			loadOrderInfoById(customerId, userId, userKey);
			$("#dlgUserInfo").modal("show");
		}
