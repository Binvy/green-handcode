...
<html>
	...
	<body>
                    ...
                    render: function (a, b, c, d) {
                        var context =
                        {
                            func: [
								...
								{"fn": "recoverPhone('"+ c.id +"')", "color": "orange", "icon": "hand-up", "tooltip": "号码回收"}
                            ]
                        };
        ...
        function recoverPhone(id) {
	   		bootbox.confirm({
				...
				callback: function(result){
	   				if(result){
						var parm = {"id":id };
					    var resData = ajaxCRUD("resPhoneNumber!recoverPhone.do", parm, true);
						if(resData.resNum == "0"){
							//table_view.fnSettings().ajax.data = {}; 
							table_view.api().ajax.reload();
						}
    	...
    	{{#each func}}
    	<a class="{{this.color}}" href="javascript:{{this.fn}}" title="{{this.tooltip}}"><i class="icon-{{this.icon}} bigger-130"></i></a>
    	{{/each}}
		</div>
		</script>
