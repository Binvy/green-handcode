...
<html>
	...
	<body class="no-skin" style="overflow: hidden;">
			...
			<div class="navbar-container" id="navbar-container">
				...
				<div class="navbar-buttons navbar-header pull-right" role="navigation">
					<ul class="ace-nav">
						<!-- manual doc -->
						<li>
							<a href="manualDown!execute.do" title="下载帮助文档">
		...
		<div class="main-container" id="main-container">
			<div class="main-container-inner"> <!-- 清除右侧滚动条:在此添加overflow属性 -->
				<div class="sidebar" id="sidebar" style="height:100%; overflow: auto;">
					<div class="sidebar-shortcuts" id="sidebar-shortcuts">
						<div class="sidebar-shortcuts-large" id="shortcuts">
						</div>
					</div>
					<ul class="nav nav-list" id="textTree"></ul>
					<!--  
				...
				<!-- <div id="content-tab" class="content-tab" style="height: 40px; width: 100%; background-color:windowframe; display: none;" >
				</div> -->
		...
		$(document).ready(function (){
			...
			// 载入快捷按钮
			var sf = function () {
				var res = ajaxCRUD("menuAccess!findTop4.do", {"obj.oper": "${session.userInfo.userName}" }, false);
				var lst = res.lst;
				...
				for (var i = 0; i < lst.length; i++) {
					var item = lst[i];
					html += '<button class="btn btn-' + styles[i] 
						+ '" onclick=goToRight("' + item.url + '","' + item.menuId + '")>'
						//+ ' oncontextmenu=setAccessMenu("' + item.url + '","' + item.menuId +'")>';
					html += '<i class="' + item.menuIcon + '"></i>';
					html += '</button>';
				}
				$("#shortcuts").empty();
				$("#shortcuts").html(html);
            ...
            /* $("#textTree > li > a").css("background-color","RGB(223,240,216)");//RGB(223,240,216)淡青色,
			$("#textTree > li > a").css("color","RGB(105,170,70)");//RGB(67,142,185)主打蓝,RGB(105,170,70)
			$("#textTree > li > a").css("font-weight","bold");
			$("#textTree > li > ul > li").css("line-hight","initial");
			$("#textTree > li > ul > li").css("background-color", "RGB(255,251,186)");
			$("#textTree > li > ul > li > a").css("font-weight","bold");
			$("#textTree > li > ul > li > a").css("color","RGB(67,142,185)");//RGB(129,191,56),RGB(105,170,70)
			$("#textTree > li > ul > li > ul > li").css("background-color", "RGB(217,239,253)");//RGB(204,204,204),RGB(197,186,164),RGB(204,255,255)
			$("#textTree > li > ul > li > ul > li").css("line-height", "initial");
			$("#textTree > li > ul > li > ul > li > a").css("font-weight", "bold");
			$("#textTree > li > ul > li > ul > li > a").css("color", "RGB(128,54,45)");//RGB(59,31,19),RGB(186,145,101) 
			//鼠标悬停字体变色
			$("#textTree > li > a:hover").css("color","#ff5722");
			$("#textTree > li > a").css("font-family","SimSun");//#ff5722:橙色 #009688:绿色 #dff0d8淡绿色
			//$("#textTree > li > a").css("color","#2196f3");
			$("#textTree > li > ul > li").css("line-hight","initial");
			*/
		});
		...
		function buildMenu(menuId, menuName, menuList){
			...
			function goToRight(url,id,pid){
				...
				var oper = "${session.userInfo.userName}";
				var parm = {
					"obj.oper": oper,
					"obj.menuId": id
					//"obj.menuId": id == undefined ? "" : id
				};
				ajaxCRUD("menuAccess!access.do", parm, false);
			}
